...
export const createTesztSchema = z.object({
  ...
  tantargyId: z.string().uuid("Érvénytelen tantárgy azonosító."),  temakorId: z.string().uuid("Érvénytelen témakör azonosító.").nullable().optional(),
...
export const updateTesztSchema = z.object({
  ...
  javasoltPerc: z.number().int().min(1).max(300).optional().nullable(),  allapot: z.enum(TESZT_ALLAPOTOK).optional(),
